...
import { escapeHtml, sanitizeRichHtml } from './core/text-volumes.js'
import {
...
function sectionHtml(title = '', html = '') {
  ...
  if (!safe) return ''
  return `<section class="rich-section"><h3>${escapeHtml(title || '正文')}</h3><div>${safe}</div></section>`
}

function richSectionsHtml(item = {}) {
  const sections = Array.isArray(item.richSections) && item.richSections.length
    ? item.richSections
    : (Array.isArray(item.sections) ? item.sections : [])
  const out = []

  if (item.descHtml) out.push(sectionHtml('描述', item.descHtml))
  else if (item.desc) out.push(sectionHtml('描述', textToHtml(item.desc)))

  for (const [idx, sec] of sections.entries()) {
    const html = sec.html || (sec.text ? textToHtml(sec.text) : '')
    if (html) out.push(sectionHtml(sec.title || sec.displayTitle || `正文 ${idx + 1}`, html))
  }

  if (item.detailHtml || item.detail) out.push(sectionHtml('角色详情', item.detailHtml || textToHtml(item.detail)))
  for (const story of item.stories || []) out.push(sectionHtml(story.name || '角色故事', story.html || textToHtml(story.text || '')))
  for (const other of item.others || []) out.push(sectionHtml(other.name || '其他', other.html || textToHtml(other.text || '')))

  for (const piece of item.pieces || []) {
    const lines = []
    if (piece.nameHtml || piece.name) lines.push(`<strong>${sanitizeRichHtml(piece.nameHtml || escapeHtml(piece.name))}</strong>`)
    if (piece.descHtml || piece.desc) lines.push(sanitizeRichHtml(piece.descHtml || textToHtml(piece.desc)))
    if (piece.storyHtml || piece.story) lines.push(sanitizeRichHtml(piece.storyHtml || textToHtml(piece.story)))
    if (lines.length) out.push(sectionHtml(piece.slot || '部位', lines.join('<br><br>')))
  }

  if (item.storyHtml || item.story) out.push(sectionHtml('武器故事', item.storyHtml || textToHtml(item.story)))
  return out.join('')
}
...
async function getEntryContent(type, id) {
  ...
  if (type === 'role') {
    ...
    return { ...meta, text: renderRoleStoryText(data, 'story'), html: richSectionsHtml(data) }
  }
  if (type === 'relic') {
    const file = path.join(relicRoot, `${slugify(meta.name)}.json`)
    const data = await readJson(file)
    return { ...meta, text: renderRelicText(data), html: richSectionsHtml(data) }
  }
  if (type === 'weapon') {
    const file = path.join(weaponRoot, `${slugify(meta.name)}.json`)
    const data = await readJson(file)
    return { ...meta, text: renderWeaponText(data), html: richSectionsHtml(data) }
  }
  if (type === 'voice') {
  ...
  const data = await readJson(file)
  const text = renderers[type](data, 'full')
  return { ...meta, text, html: richSectionsHtml(data) }
}
...
.viewer.rich{white-space:normal}.viewer.rich .rich-section{padding:0 0 18px;margin:0 0 18px;border-bottom:1px solid var(--line)}.viewer.rich .rich-section:last-child{border-bottom:0;margin-bottom:0;padding-bottom:0}.viewer.rich h3{margin:0 0 12px;font-size:18px;color:var(--ink)}.viewer.rich p,.viewer.rich div{margin:0 0 10px}.viewer.rich span,.viewer.rich strong,.viewer.rich em,.viewer.rich u,.viewer.rich s{color:inherit}
...
async function openEntry(type,id){try{const item=await api('/api/content?type='+type+'&id='+encodeURIComponent(id));const body=item.html?'<div class="viewer rich">'+item.html+'</div>':'<div class="viewer">'+esc(item.text||'暂无内容')+'</div>';el('app').innerHTML='<div class="toolbar"><div><b>'+esc(item.name)+'</b><div class="small">'+TYPES.find(t=>t.key===type).label+'</div></div><button class="secondary" onclick="openType(currentType,currentPage)">返回</button><button class="accent" onclick="startUpdate(currentType)">更新分量</button></div>'+body}catch(e){showError(e)}}
