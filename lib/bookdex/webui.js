...
async function getTypeEntries(type) {
  if (type === 'book') {
    ...
    return (index.books || []).map(item => ({ type, id: String(item.source || '').replace(/^wiki:/, ''), name: item.title, file: item.file }))
  }
  if (type === 'role') {
...
async function getEntryContent(type, id) {
  ...
  if (type === 'book') {
    ...
    return { ...meta, text: fss.existsSync(file) ? await fs.readFile(file, 'utf8') : '' }
  }
  if (type === 'role') {
