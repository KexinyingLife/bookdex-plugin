    ...
    try {
      ...
      if (page) {
        ...
        if (detail || stories.length || others.length) {
          ...
          roles.push({ id, name: roleName, alias: item.alias, storyCount: stories.length, otherCount: others.length, selectorSig: sig })
        }
      }
    } catch (error) {
  ...
  if (dryRun) {
    ...
    for (const itemInfo of changedItems) {
      try {
        ...
        if (page) effectiveChanged += 1
      } catch (error) {
        await emitProgress(onError, { type: 'plot', done: effectiveChanged + 1, total: changedItems.length, name: itemInfo.name, error })
      }
    }
    return { total: items.length + effectiveChanged, updated: effectiveChanged + removed.length }
  }
  ...
  for (const itemInfo of changedItems) {
    ...
    try {
      ...
      if (page) {
        ...
        items.push({ id, name, file: fileName, alias: item.alias, category, sectionCount: (item.sections || []).length, selectorSig: sig })
        saved += 1
      }
    } catch (error) {
...
async function fetchBooksFromWiki({ onProgress, onError, dryRun = false } = {}) {
  ...
  for (const it of map.values()) {
    ...
    const canReuse = prev && prevFile && fss.existsSync(prevFile) && (prev.selectorSig === sig || !prev.selectorSig)
    if (canReuse) {
      if (prev.file && prev.file !== out && prevFile && fss.existsSync(prevFile)) {
        await fs.rename(prevFile, filePath)
      }
      index.books.push({ ...prev, title: name, file: out, source: `wiki:${id}`, selectorSig: sig })
    } else {
    ...
    try {
      ...
      if (page) {
        ...
        if (text) {
          ...
          index.books.push({ title: name, file: out, source: `wiki:${id}`, selectorSig: sig })
        }
      }
    } catch (error) {
