...
async function fetchRoleStoryAll({ onProgress, onError } = {}) {
  await ensureDirs()
  ...
  for (const it of roleMap.values()) {
    ...
    if (done % 100 === 0 || done === total) await emitProgress(onProgress, { type: 'role', done, total })
  }
...
async function fetchRelicAll({ onProgress, onError } = {}) {
  await ensureDirs()
  ...
  for (const it of setMap.values()) {
    ...
    if (done % 100 === 0 || done === total) await emitProgress(onProgress, { type: 'relic', done, total })
  }
...
async function fetchWeaponAll({ onProgress, onError } = {}) {
  await ensureDirs()
  const map = new Map()
  for (let page = 1; page <= 20; page++) {
    const u = new URL('https://act-api-takumi.mihoyo.com/common/blackboard/ys_obc/v1/content/selector')
  ...
  for (const it of map.values()) {
    ...
    if (done % 100 === 0 || done === total) await emitProgress(onProgress, { type: 'weapon', done, total })
  }
...
async function fetchPlotAll({ onProgress, onError } = {}) {
  await ensureDirs()
  const map = new Map()
  ...
  for (const it of map.values()) {
    ...
    if (done % 100 === 0 || done === total) await emitProgress(onProgress, { type: 'plot', done, total })
  }
...
async function fetchVoiceAll({ onProgress, onError } = {}) {
  await ensureDirs()
  const roleMap = new Map()
  for (let page = 1; page <= 20; page++) {
    const u = new URL('https://act-api-takumi.mihoyo.com/common/blackboard/ys_obc/v1/content/selector')
  ...
  for (const it of roleMap.values()) {
    ...
    if (done % 100 === 0 || done === total) await emitProgress(onProgress, { type: 'voice', done, total })
  }
...
async function fetchBooksFromWiki({ onProgress, onError } = {}) {
  await ensureDirs()
  const map = new Map()
  for (let page = 1; page <= 20; page++) {
    const u = new URL('https://act-api-takumi.mihoyo.com/common/blackboard/ys_obc/v1/content/selector')
  ...
  for (const it of map.values()) {
    ...
    if (done % 100 === 0 || done === total) await emitProgress(onProgress, { type: 'book', done, total })
  }
