...
const pipelineStateFile = path.join(cacheRoot, 'pipeline.json')
...
const mapRoot = path.join(dataRoot, 'maps')
const mapIndexFile = path.join(mapRoot, 'index.json')
const anecdoteRoot = path.join(dataRoot, 'anecdotes')
const anecdoteIndexFile = path.join(anecdoteRoot, 'index.json')
const cardRoot = path.join(dataRoot, 'cards')
const cardIndexFile = path.join(cardRoot, 'index.json')
const backpackRoot = path.join(dataRoot, 'backpacks')
const backpackIndexFile = path.join(backpackRoot, 'index.json')
...
async function ensureDirs() {
  ...
  await fs.mkdir(anecdoteRoot, { recursive: true })
  await fs.mkdir(cardRoot, { recursive: true })
  await fs.mkdir(backpackRoot, { recursive: true })
}
...
async function loadMapIndex() {
  try { return JSON.parse(await fs.readFile(mapIndexFile, 'utf8')) } catch { return { items: [] } }
}

async function loadAnecdoteIndex() {
  try { return JSON.parse(await fs.readFile(anecdoteIndexFile, 'utf8')) } catch { return { items: [] } }
}

async function loadCardIndex() {
  try { return JSON.parse(await fs.readFile(cardIndexFile, 'utf8')) } catch { return { items: [] } }
}

async function loadBackpackIndex() {
  try { return JSON.parse(await fs.readFile(backpackIndexFile, 'utf8')) } catch { return { items: [] } }
}

/** 记录剧情/地图/逸闻的 interactive_dialogue 解析版本（见 core.js INTERACTIVE_DIALOGUE_PIPELINE_VERSION） */
async function loadPipelineState() {
  try {
    const raw = await fs.readFile(pipelineStateFile, 'utf8')
...
async function savePipelineModuleVersion(moduleKey, version) {
  await fs.mkdir(cacheRoot, { recursive: true })
  const cur = await loadPipelineState()
  cur[moduleKey] = version
  await fs.writeFile(pipelineStateFile, JSON.stringify(cur, null, 2), 'utf8')
}
