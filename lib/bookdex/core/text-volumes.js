...
function parseBookDescriptionFromMaterial(module = {}) {
    ...
    for (const c of comps) {
        ...
        try {
            ...
            if (materials.key && materials.value) {
                const val = cleanPlotText(htmlToText(materials.value))
                if (val) lines.push(`${materials.key}：\n\t${val.replace(/\n/g, "\n\t")}`)
            }

            const attrs = Array.isArray(d?.attr) ? d.attr : []
            for (const attr of attrs) {
                if (!attr.key || !attr.value) continue
                const list = Array.isArray(attr.value) ? attr.value : [attr.value]
                const val = cleanPlotText(list.map(x => htmlToText(x)).filter(Boolean).join("\n"))
                if (val) lines.push(`${attr.key}：\n\t${val.replace(/\n/g, "\n\t")}`)
            }
