...
import { parseGenericPlotComponent, parseGenericPlotComponentRich } from './parse-plot.js'
...
function parseMapPage(page = {}) {
  ...
  for (const module of modules) {
    ...
    for (const comp of (module.components || [])) {
      ...
      const html = parseInteractiveDialogueRich(comp)
      if (!text) continue
      sections.push({
...
function parseModuleDialogueLikeRich(module = {}) {
  const texts = []
  for (const comp of (module.components || [])) {
    const txt = parseGenericPlotComponentRich(comp)
    if (txt) texts.push(txt)
  }
  return texts.join('\n')
}
  ...
  for (const m of modules) {
    ...
    if (cids.includes('interactive_dialogue')) {
      for (const comp of (m.components || [])) {
        ...
        const html = parseInteractiveDialogueRich(comp)
        if (!txt) continue
        sections.push({ title: name, text: txt, html })
      }
      continue
    }
    ...
    if (cids.includes('rich_base_info')) {
      ...
      const html = parseModuleDialogueLikeRich(m)
      if (txt) sections.push({ title: name, text: txt, html })
    }
  }
  return sections
}
    ...
    for (const comp of (m.components || [])) {
      ...
      for (const table of (data.tables || [])) {
        ...
        for (const row of (table.row || [])) {
          ...
          const html = cells.map(cell => sanitizeRichHtml(cell || '').trim()).filter(Boolean).join('<br>')
          if (txt) lines.push({ text: txt, html })
        }
        const text = cleanPlotText(lines.map(line => line.text).join('\n\n'))
        const html = lines.map(line => line.html).filter(Boolean).join('<br><br>')
        if (text) sections.push({ title, text, html })
      }
    }
  }
  return sections
  ...
  for (const m of modules) {
    for (const comp of (m.components || [])) {
      ...
      for (const attr of attrs) {
        ...
        if (!txt) continue
        descHtml = vals.map(v => sanitizeRichHtml(v)).filter(Boolean).join('<br>')
        lines.push(txt)
      }
...
function parseBackpackReadableSections(page = {}, desc = '') {
  ...
  const add = (title, htmlOrText) => {
    ...
    const html = sanitizeRichHtml(htmlOrText)
    ...
    seen.add(key)
    sections.push({ title: title || `正文 ${sections.length + 1}`, text: clean, html, images })
  }
