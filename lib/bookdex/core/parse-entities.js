import { pickSectionText, pickSectionHtml, htmlToText, sanitizeRichHtml } from './text-volumes.js'
...
function extractRoleStory(page = {}) {
  ...
  for (const m of modules) {
    if (/^角色故事\d+$/.test(m.name || '')) {
      const t = pickSectionText(m)
      if (t) stories.push({ name: m.name, text: t, html: pickSectionHtml(m) })
    }
  }
  ...
  for (const n of pickNames) {
    const m = modules.find(x => x.name === n)
    const t = pickSectionText(m)
    if (t) other.push({ name: n, text: t, html: pickSectionHtml(m) })
  }
  ...
  if (other.length < 2) {
    ...
    for (const m of modules) {
      ...
      other.push({ name, text: t, html: pickSectionHtml(m) })
...
function parseRelicPiece(module = {}) {
  ...
  return {
    ...
    nameHtml: sanitizeRichHtml(pieceName),
    desc: htmlToText(descHtml),
    descHtml: sanitizeRichHtml(descHtml),
