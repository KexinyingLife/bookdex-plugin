...
function buildBookSectionsFromEntryPage(page = {}) {
  ...
  const volumeAcquireMap = new Map()
  const exactNameAcquireMap = new Map()
  const volumeDescQueue = []
  for (const m of modules) {
    const item = parseBookDescriptionFromMaterial(m)
    if (!item?.desc && !item?.acquireMethod) continue
    ...
    if (item.acquireMethod) {
      if (key && !volumeAcquireMap.has(key)) volumeAcquireMap.set(key, item.acquireMethod)
      if (item.rawName && !exactNameAcquireMap.has(item.rawName)) exactNameAcquireMap.set(item.rawName, item.acquireMethod)
    }
    volumeDescQueue.push({ key: key || '', desc: item.desc || '', acquireMethod: item.acquireMethod || '' })
  }
  ...
  for (const m of orderedModules) {
    ...
    if (n && exactNameDescMap.has(n) && !usedExactNames.has(n)) {
      ...
      acquireMethod = exactNameAcquireMap.get(n) || ''
      usedExactNames.add(n)
    } else if (matchedKey) {
      desc = volumeDescMap.get(matchedKey) || ''
      acquireMethod = volumeAcquireMap.get(matchedKey) || ''
    }

    if (!desc && !acquireMethod && !labelKey) {
      ...
      if (nextIdx >= 0) {
        ...
        desc = next.desc
        acquireMethod = next.acquireMethod
...
async function buildBookTextFromEntryPage(page = {}) {
  ...
  for (const s of sections) {
    if (s.displayTitle) {
      let header = `【${s.displayTitle}】`
      if (s.acquireMethod) header += `\n【获得方式】\n${s.acquireMethod}`
      if (s.desc) header += `\n【描述】\n${s.desc}`
      segs.push(`${header}\n\n${s.text}`)
...
function extractBookDescriptionFromEntryPage(page = {}) {
  ...
  for (const m of page.modules || []) {
    const item = parseBookDescriptionFromMaterial(m)
    if ((!item?.desc && !item?.acquireMethod) || seen.has(item.desc)) continue
    seen.add(item.desc)
    rows.push({
      label: normalizeBookVolumeToken(item.label) || item.rawName || '',
      desc: item.desc,
      acquireMethod: item.acquireMethod || ''
    })
  }

  if (!rows.length) {
    const pageDesc = cleanPlotText(page.desc || '')
    const name = String(page.name || '').trim()
    return pageDesc && pageDesc !== name ? pageDesc : ''
  }
  const formatRow = (item) => {
    let text = item.label ? `${item.label}：${item.desc}` : item.desc
    if (item.acquireMethod) text += `\n获得方式：${item.acquireMethod}`
    return text
  }
  if (rows.length === 1) return formatRow(rows[0])
