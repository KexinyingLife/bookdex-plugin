...
import { BOOK_TEXT_SCHEMA_VERSION, PLOT_TEXT_SCHEMA_VERSION, MAP_TEXT_SCHEMA_VERSION, ROLE_STORY_SCHEMA_VERSION, BACKPACK_TEXT_SCHEMA_VERSION } from './constants.js'
...
async function updateOneBackpackByName(rawName = '') {
  ...
  const data = { id, name: finalName, file: fileName, alias: [normalizeRoleName(finalName)], desc, sections, imageUrls, searchText }
  await fs.writeFile(filePath, JSON.stringify(data, null, 2), 'utf8')
  ...
  upsertById(arr, {
    ...
    contentHash: stableHash({ desc, sections })
  })
  arr.sort((a, b) => a.name.localeCompare(b.name, 'zh-Hans-CN') || String(a.id).localeCompare(String(b.id)))
