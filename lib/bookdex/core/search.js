...
import { renderBookTextWithDescription } from './inbox-books.js'
...
async function runBookDexTextSearch(keyword, types = SEARCH_TYPES) {
  ...
  if (selected.includes('book')) {
    ...
    for (const b of bi.books || []) {
      ...
      const text = renderBookTextWithDescription(b.title, await fs.readFile(full, 'utf8'), b.desc)
      hitRow(rows, { type: 'book', id: String(b.source || '').replace(/^wiki:/, ''), name: b.title, file: b.file, desc: b.desc || '' }, key, text, b.title.includes(key))
    }
  }
